...
class Argument(object):
    ...
    def convert(self, value, op):
        ...
        #  and check if we're expecting a filestorage and
        #  haven't overridden `type`
        #  (required because the below
        #  instantiation isn't valid for FileStorage)
    ...
    def handle_validation_error(self, error, bundle_errors):
        ...
        error_msg = self.help.format(
            error_msg=error_str
        ) if self.help else error_str
        #
        msg = {self.name: error_msg}

        if current_app.config.get("BUNDLE_ERRORS", False) or bundle_errors:
            return error, msg
        raise exc.InvalidArgument(self.name, error_msg)
            ...
            if name in source:
                ...
                for value in values:
                    ...
                    if self.choices and value not in self.choices:
                        config_bundle_errors = current_app.config.get(
                            "BUNDLE_ERRORS", False
                        )
                        if config_bundle_errors or bundle_errors:
                            return self.handle_validation_error(
                                ValueError(u"{0} is not a valid choice".format(
                                    value)), bundle_errors)
                        self.handle_validation_error(
                            ValueError(
                                u"{0} is not a valid choice".format(
                                    value
                                )
                            ), bundle_errors
                        )
...
class RequestParser(object):
    ...
    :param bool trim: If enabled, trims whitespace on all arguments in this
        parser
    :param bool bundle_errors: If enabled, do not abort
        when first error occurs, return a dict with the name of the argument
        and the error message to be bundled and return all validation errors
    """

    def __init__(
            self, argument_class=Argument, namespace_class=Namespace,
    ...
    def parse_args(self, req=None, strict=False):
        ...
        req.unparsed_arguments = dict(
            self.argument_class('').source(req)
        ) if strict else {}
        #
        errors = {}
    ...
    def copy(self):
        """
        Creates a copy of this RequestParser
